...
parser.add_argument("--ls-enabled", type=int, default=1,
                    help="Use LSLinear (Low-rank + Sparse) instead of nn.Linear (1=on, 0=off)")
parser.add_argument("--ls-num-blocks", type=int, default=16,
                    help="Number of diagonal blocks in sparse component")
...
def flash_attn_func(q, k, v, causal=False, window_size=(-1, -1)):
    """Flash Attention for training. FA3 on Hopper, PyTorch SDPA fallback elsewhere."""
    if _fa3 is not None:
        return _fa3.flash_attn_func(q, k, v, causal=causal, window_size=window_size)
    # Fallback: PyTorch scaled_dot_product_attention (for non-H100 GPUs)
    # q,k,v: (B, T, H, D) -> need (B, H, T, D)
    q = q.transpose(1, 2)
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
...
class LSLinearCompiled(nn.Module):
    ...
    BMM path, torch.compile friendly. sparse_weight stored 2D for Muon."""
...
def make_linear(in_features, out_features, bias=False, use_ls=True):
    """Factory: LSLinearCompiled or nn.Linear based on global config."""
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.c_q = make_linear(self.n_embd, self.n_head * self.head_dim, use_ls=True)
        self.c_k = make_linear(self.n_embd, self.n_kv_head * self.head_dim, use_ls=False)  # K needs full rank
        self.c_v = make_linear(self.n_embd, self.n_kv_head * self.head_dim, use_ls=False)  # V needs full rank
        self.c_proj = make_linear(self.n_embd, self.n_embd, use_ls=True)
...
class MLP(nn.Module):
    def __init__(self, config):
        ...
        self.c_gate = make_linear(config.n_embd, hidden, use_ls=True)
        self.c_fc = make_linear(config.n_embd, hidden, use_ls=True)
        self.c_proj = make_linear(hidden, config.n_embd, use_ls=True)
        self.resid_dropout = nn.Dropout(config.dropout)
...
def _init_ls_uniform(layer, bound):
    """Init LSLinearCompiled sparse blocks with uniform, A=0, B=kaiming."""
...
def _init_ls_normal(layer, std):
    """Init LSLinearCompiled sparse blocks with normal, A=0, B=kaiming."""
    W = layer.sparse_weight.data.reshape(layer.num_blocks, layer.block_out, layer.block_in)
    for i in range(layer.num_blocks):
...
def _init_layer(layer, mode, scale):
    """Init either nn.Linear or LSLinearCompiled."""
...
class GPT(nn.Module):
    ...
    def init_weights(self):
        ...
        if self.mtp_weight > 0:
            ...
            if isinstance(self.mtp_proj, LSLinearCompiled):
                _init_ls_uniform(self.mtp_proj, s)
            else:
                torch.nn.init.uniform_(self.mtp_proj.weight, -s, s)
        for block in all_blocks:
            _init_layer(block.attn.c_q, 'uniform', s)
            _init_layer(block.attn.c_k, 'uniform', s)
            _init_layer(block.attn.c_v, 'uniform', s)
            _init_layer(block.attn.c_proj, 'normal', normal_std)
            _init_layer(block.mlp.c_gate, 'uniform', s)
            _init_layer(block.mlp.c_fc, 'uniform', s)
            _init_layer(block.mlp.c_proj, 'normal', normal_std)
...
if _fa3 is not None:
    ...
    print0("FA3 not available — using PyTorch SDPA fallback (window attention disabled)")
...
if ls_params > 0:
    print0(f"LSLinear: {ls_params:,} params in L+S layers | Dense: {dense_linear_params:,} in nn.Linear")
    print0(f"LSLinear config: blocks={LS_NUM_BLOCKS}, rank={LS_RANK}, enabled={LS_ENABLED}")
